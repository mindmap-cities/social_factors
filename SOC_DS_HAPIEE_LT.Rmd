...
#HAPIEE_W2 
opal.assign.table.tibble(erasmus_opal, 'HAPIEE_W2', 'HAPIEE.HAPIEE_W2',variables = list('country','w2chldno_lt','p58_lt','p57_lt','p55_lt'))
...
```{r, echo=FALSE}
soc_HAPIEE_LT_0 <- data.frame(id = HAPIEE_W2$id )
```
...
```{r, echo=TRUE}
#w2child_lt 
table(HAPIEE_W2$w2chldno_lt) 

soc_HAPIEE_LT_0$soc_sn_num_children_0 <- as.integer(HAPIEE_W2$w2chldno_lt)

table(soc_HAPIEE_LT_0$soc_sn_num_children_0) 
...
soc_HAPIEE_LT_0$soc_sn_children_rank_0 <- ifelse(HAPIEE_W2$w2chldno_lt=='0',0L, 
                                             ifelse(HAPIEE_W2$w2chldno_lt=='1' | HAPIEE_W2$w2chldno_lt=='2',1L,
                                                    ifelse(HAPIEE_W2$w2chldno_lt >'2',2L, NA)))

table(soc_HAPIEE_LT_0$soc_sn_children_rank_0)
...
```{r, echo=TRUE}
#p58_lt
table(HAPIEE_W2$p58_lt) 
# should we go back and chang categories for all other DataSchemas?

soc_HAPIEE_LT_0$soc_sn_freq_friends_inperson_0 <-  ifelse(HAPIEE_W2$p58_lt=='1',2L, 
                                                     ifelse(HAPIEE_W2$p58_lt=='2' | HAPIEE_W2$p58_lt=='3',3L,
                                                       ifelse(HAPIEE_W2$p58_lt =='4',4L, 
                                                         ifelse(HAPIEE_W2$p58_lt =='5' | HAPIEE_W2$p58_lt =='5',5L,NA))))

table(soc_HAPIEE_LT_0$soc_sn_freq_friends_inperson_0) 
``` 
### **Variable label**:   In-person contact frequency with friends        
...
```{r, echo=TRUE}
#p57_lt
#s68 and s67 are missing from LT set to I cannot use these to calculate DAILY contact, should we go back and change for all cohorts the categories 

table(HAPIEE_W2$p57_lt) 

soc_HAPIEE_LT_0$soc_sn_freq_relatives_0 <-  ifelse(HAPIEE_W2$p57_lt=='1'|HAPIEE_W2$p57_lt=='2',1L, 
                                                     ifelse(HAPIEE_W2$p57_lt=='3',2L,
                                                       ifelse(HAPIEE_W2$p57_lt =='4',3L, 
                                                         ifelse(HAPIEE_W2$p57_lt =='5' | HAPIEE_W2$p57_lt =='6',4L,NA))))
table(soc_HAPIEE_LT_0$soc_sn_freq_relatives_0)
...
```{r, echo=TRUE}
#p55_lt
table(HAPIEE_W2$p55_lt) #1 yes, a member  2 no

soc_HAPIEE_LT_0$soc_sp_general_0 <- ifelse(HAPIEE_W2$p55_lt=='1',1L, 
                                                    ifelse(HAPIEE_W2$p55_lt =='2',0L, NA))

table(soc_HAPIEE_LT_0$soc_sp_general_0)
...
**Harmonization status**:  impossible  complete
**Harmonization comment**: Participants that had low (never) contact frequency with friends (in person) and with relatives (2 items), and reported no participation were categorized as isolated. We did not use a low social network as a criteria (alongside no participation) for social isolation, as in GLOBE and RECORD, since the social network item harmonized in HAPIEE is ranking of children and all participants have at least 1 child, which might or might not be alive. 
**R script**:  
```{r, echo=TRUE}
table(soc_HAPIEE_LT_0$soc_sn_children_rank_0)
table(soc_HAPIEE_LT_0$soc_sp_general_0)
soc_sn_num_children_0
# so no body is isolated since no one is childless but perhaphs the child has passed away, one way is to use contact frequency to identify isolated persons, checking with supervisor

table(soc_HAPIEE_LT_0$soc_sn_freq_friends_inperson_0)
soc_HAPIEE_LT_0$soc_sp_isolation_0 <- ifelse((soc_HAPIEE_LT_0$soc_sn_freq_friends_inperson_0  == 5  | soc_HAPIEE_LT_0$soc_sn_freq_relatives_0 ==4) & soc_HAPIEE_LT_0$soc_sp_general_0 ==0,1L,                                         ifelse(soc_HAPIEE_LT_0$soc_sn_freq_friends_inperson_0 <=4  | soc_HAPIEE_LT_0$soc_sn_freq_relatives_0 <4 & soc_HAPIEE_LT_0$soc_sp_general_0 == 1,0L,NA ))

table(soc_HAPIEE_LT_0$soc_sp_isolation_0)
